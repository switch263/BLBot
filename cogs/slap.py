...
class Slap(commands.Cog):
    ...
    async def on_ready(self):
        ...
        try:
            if self.stats:
                self.stats.register_cog(self.stats.db_path, "slap", ["slapper", "slapped"])
    ...
    @commands.command(name='slap', help='Slap someone in the channel. If no one is mentioned, a random person is slapped. If "kev2tall" is mentioned, the author gets slapped. If bot is mentioned explicitly, the bot will reply with a snarky message and slap the author with a generated slap item.')
    async def slap(self, ctx, target: discord.Member = None):
        ...
        if target == self.bot.user:
            if self.stats:
                try:
                    await self.stats.update_stats("slap", userid=str(ctx.author.id), slapper=1, slapped=1)
                    await self.stats.update_stats("slap", userid=str(self.bot.user.id), slapper=1, slapped=0)
                except:
        ...
        if self.stats:
            try:
                await self.stats.update_stats("slap", userid=str(ctx.author.id), slapper=1)
                await self.stats.update_stats("slap", userid=str(target.id), slapped=1)
                logger.debug("Updating slap stats")
            except:
                logger.debug("ERROR: Updating slap stats")

  ...
  async def quote(self, ctx, subcommand: str = "get", *, quote: str = None):
    ...
    if subcommand.lower() == 'add':
      ...
      if quote:
        if len(quote.strip()) > 0:
          ...
          rowid = cur.lastrowid
          dbconn.commit()
          await ctx.send(":microphone: " + "[" + str(rowid) + "] " + quote +
                         " added to database")
          dbconn.close()
    ...
    if subcommand.lower() == "get":
      try:
        ...
        dbconn = sqlite3.connect(dbfile)
        cur = dbconn.cursor()
        randquote = cur.execute("SELECT * FROM quotes WHERE rowid = ?;",
                                [quote])
        ...
        randquote = randquote.fetchall()[0][0]
        rowid = cur.execute("SELECT rowid FROM quotes WHERE quote like (?)",
                            [randquote])
